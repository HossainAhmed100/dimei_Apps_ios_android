...
const AllDevice = ({navigation}) => {
  ...
  const { isLoading, isError, data: myDevice = [], refetch: refetchMyDevice } = useQuery({ 
    ...
    queryFn: async () => {
      const res = await axios.get(`http://192.168.0.163:5000/myalldevice/${user?.userEmail}`);
      return res.data;
    } 
  })
  ...
  const { isLoading: reciveDeviceLoading, data: reciveDevice = [], refetch } = useQuery({ 
    ...
    queryFn: async () => {
      const res = await axios.get(`http://192.168.0.163:5000/reciveTransferDevice/${user?.userEmail}`);
      return res.data;
    } 
  })
