...
const AddDeviceInput = ({navigation, route}) => {
  ...
  const { data: devciePreview = []} = useQuery({ 
    ...
    queryFn: async () => {
      const res = await axios.get(`http://192.168.0.181:5000/checkDeviceImeiNum/${deviceImeiInput}`);
      return res.data;
    } 
  })
  ...
  try {
      const response = await axios.post('http://192.168.0.181:5000/addNewDevice', {deviceInfos})
