...
const VerifyDeviceAcceft = ({navigation, route})  => {
    ...
    const { isLoading, isError, data: accevtDevice = [], error } = useQuery({ 
        ...
        queryFn: async () => {
        const res = await axios.get(`http://192.168.1.5:5000/getTransferDeviceDetails/${transferDeviceId}`);
        return res.data;
        } 
    })
    ...
    const onSubmit = async (data) => {
        ...
        if(user){
        await axios.put(`http://192.168.1.5:5000/devicesecretCodetst/`, {deviceTestInfo})
        .then((res) => {
