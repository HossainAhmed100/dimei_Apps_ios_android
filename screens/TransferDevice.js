...
const TransferDevice =  ({navigation, route}) => {
    ...
    const { isLoading, isError, data: myDevice = [], error } = useQuery({ 
        ...
        queryFn: async () => {
        const res = await axios.get(`http://192.168.1.5:5000/getSingleDevice/${deviceId}`);
        return res.data;
        } 
    })
     ...
     const onSubmit = async (data) => {
        ...
        if(user){
        await axios.put(`http://192.168.1.5:5000/devicetransferStatusUpdate/`,{infoData})
        .then((res) => {
        if (res.data.modifiedCount === 1){
            try{
              axios.post(`http://192.168.1.5:5000/reciveTransferDevice/`, {transferDeviceInfo})
              .then((res) => {
